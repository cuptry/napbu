...
# 시작 지점
def startcalcunapbu():
    number = input("날짜를 입력하세요: ") # 원하는 날짜 입력, 추후 기본은 초기화, 입력시에 재초기화
    number = datetime.datetime.strptime(number, "%Y%m%d") # 입력받은 날짜를 datetime으로 변환    
    calcunapbustart(number)
...
def calcunapbustart(now):
    ...
    getdatesholidays(now, holidays)  # 납부 날짜 계산 함수 호출
